...
public class MtoOrderVoidOp extends Operation{
	@Inject
	protected TransactionScope _transactionScope;
	private static final Logger logger_ = LogManager.getLogger(MtoOrderVoidOp.class);
	public ORPOSExecuteOutput voidOrder() {
		ORPOSExecuteOutput result = null;
	
		MtoOrderList orderSelected=  _transactionScope.getValue(MtoValueKeys.SELECTED_SIEBEL_ORDER);
			try {
				result = MtoOrderServiceHelper.getInstance().orposExecuteVoid(orderSelected);
	...
	public IOpResponse handleOrderLookupWSCallSearch() {
	 ORPOSExecuteOutput request = voidOrder();
