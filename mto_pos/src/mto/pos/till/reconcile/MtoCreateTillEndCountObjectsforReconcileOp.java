...
public class MtoCreateTillEndCountObjectsforReconcileOp extends AbstractCreateTillCountObjectsOp{

	 private static final Logger logger_ = LogManager.getLogger(MtoCreateTillEndCountObjectsforReconcileOp.class);
	    @Inject
	       ...
	       var6 = transaction.getTenderTypeCounts().iterator();      while(true) {         while(var6.hasNext()) {            ITenderTypeCount tenderTypeCount = (ITenderTypeCount)var6.next();
	             ...
	             if (!tenderCountList.isEmpty()) {
	                ...
	                Iterator var11 = tenderCountList.iterator();               while(var11.hasNext()) {                  ITenderCount tenderCount = (ITenderCount)var11.next();
	                   ...
	                   ISessionTender sessionTender = this._tillHelper.getSessionTender(storeSafeSession, tenderCount.getTenderId(), Long.valueOf((long)workStationID), this.getSourceDescription());
	                ...
	                if (total.compareTo(tenderTypeCount.getAmount()) < 0) {
	                   ...
	                   List<Tender> tenders = this._tenderHelper.getAvailableIncludeInTypeCountTendersForCategory(this._transDateProvider, TenderCategory.forName(tenderTypeCount.getTenderTypeCode()), (long)workStationID);
	 
	 
	 
	                   if (tenders != null && tenders.size() > 0) {
	                      ISessionTender sessionTender = this._tillHelper.getSessionTender(storeSafeSession, ((Tender)tenders.get(0)).getTenderId(), Long.valueOf((long)workStationID), this.getSourceDescription());
