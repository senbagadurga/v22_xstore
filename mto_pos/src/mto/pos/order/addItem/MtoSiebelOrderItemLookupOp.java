...
/* Start Task 100597 
 * Item Not on File prompt appearing for 
 * transaction type available in DB*/
public class MtoSiebelOrderItemLookupOp extends Operation {
	@Inject
	protected TransactionScope _transactionScope;
	 private static final Logger logger_ = LogManager.getLogger(MtoSiebelOrderItemLookupOp.class);
	 private static MtoSiebelOrderItemLookupOp INSTANCE;
		@Inject
		private StationState stationState;
		private static final Logger _logger = LogManager.getLogger(MtoSiebelOrderItemLookupOp.class);
		MtoOrderList orderSelected = _transactionScope.getValue(MtoValueKeys.SELECTED_SIEBEL_ORDER);
		public static MtoSiebelOrderItemLookupOp getINSTANCE() {
			return INSTANCE;
		}

		public static void setINSTANCE(MtoSiebelOrderItemLookupOp iNSTANCE) {
			INSTANCE = iNSTANCE;
		}
	
	public ORPOSQueryPageOutput itemLookupInfo(String argLoginUserId,String argItemId) {
		ORPOSQueryPageOutput result = null;
		orderSelected.getId();
		if(!StringUtils.isEmpty(argLoginUserId)) {
			try {
				result = MtoOrderServiceHelper.getInstance().orposQueryPage(argLoginUserId,argItemId);
	...
	//	setScopedValue(MtoValueKeys.SIEBEL_ORDER_FLOW_COMPLETE, false);
		IOpResponse response = handleItemLookupWSCallSearch();
		
		return response;
	}
	
	public IOpResponse handleItemLookupWSCallSearch() {
		...
		String loginUserId = null;
			...
			ORPOSQueryPageOutput request = itemLookupInfo(loginUserId,itemId);
	...
	private IOpResponse handleResponseObject(ORPOSQueryPageOutput argResponseObject) {
		...
		try// TODO Auto-generated method stub
		{
			if (argResponseObject == null) {
				...
				int maxOrders = Integer.parseInt(MtoConfigurationMgr.getSiebelServiceOrderDetailPageSize());
				...
				_transactionScope.setValue(MtoValueKeys.SIEBEL_ORDER_INQUIRY_RESPONSE_ITEM, argResponseObject);
				...
				if (orders != null) {
					...
					_transactionScope.setValue(MtoValueKeys.SIEBEL_ORDER_INQUIRY_RESULTS_ITEM, externalOrders);
				}
